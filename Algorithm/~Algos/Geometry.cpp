...
namespace Geometry {
  ...
  vector<point> intersect(segment s1, segment s2) {
      ...
      long double x2 = (d.x - c.x);
      long double y2 = (d.y - c.y);
      long double u1 = (-y1 * (a.x - c.x) + x1 * (a.y - c.y)) / (-x2 * y1 + x1 * y2);
      long double u2 = (x2 * (a.y - c.y) - y2 * (a.x - c.x)) / (-x2 * y1 + x1 * y2);
  ...
  ld distance(point a, point b) {
    return sqrt((b.x - a.x) * (b.x - a.x) + (a.y - b.y) * (a.y - b.y));
  }
}